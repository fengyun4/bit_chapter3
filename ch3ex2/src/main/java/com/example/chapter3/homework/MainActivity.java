...
public class MainActivity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator animator_1x=ObjectAnimator.ofFloat(target,"scaleX",1,2,1);
        //x轴缩放，scaleX
        ObjectAnimator animator_1y=ObjectAnimator.ofFloat(target,"scaleY",1,2,1);
        //y轴缩放，scaleY
        animator_1x.setDuration(500);
        //运行时长，500毫秒

        //animator_1x.setStartDelay(500);
        // 设置动画延迟播放时间
        animator_1x.setRepeatCount(ObjectAnimator.INFINITE);
        //设置动画重复播放次数,infinite时,动画无限重复
        animator_1x.setRepeatMode(ObjectAnimator.REVERSE);
        //设置重复播放动画模式
        // ValueAnimator.RESTART(默认):正序重放
        // ValueAnimator.REVERSE:倒序回放

        animator_1y.setDuration(500);
        animator_1y.setRepeatCount(ObjectAnimator.INFINITE);
        animator_1y.setRepeatMode(ObjectAnimator.REVERSE);
        // TODO 2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环

        ObjectAnimator animator_2=ObjectAnimator.ofFloat(target,"alpha",1f,0.5f,1f);
        animator_2.setDuration(5000);
        ...
        animatorSet.playTogether(animator1,animator_1x,animator_1y,animator_2);
        //playTogether，联合动画
        animatorSet.start();
    }
}
